...
import { getFolderByPurpose, slugifyFilename, getPurposeByFolder, } from "./file.utils.js";
import { env } from "../../config/env.js";
import { HttpError } from "../common/errors.js";
export async function initUpload(req, res) {
    const { fileName, mimeType, size, purpose, checksum } = req.body;
    ...
    // Get presigned URL
    const { url, key: finalKey } = await storageService.getPresignedUploadUrl("", // prefix is already in key
    key, mimeType, size, checksum);
    res.json({
        url,
        key: finalKey,
    });
...
export async function confirmUpload(req, res) {
    ...
    if (checksum) {
        if (metadata.etag !== checksum) {
            ...
            throw HttpError.badRequest(`Checksum mismatch. Expected ${checksum}, got ${metadata.etag}`, "INTEGRITY_CHECK_FAILED");
        }
    }
